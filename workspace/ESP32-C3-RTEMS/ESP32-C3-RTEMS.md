...
**Status: draft/unverified.** `esp32c3db` was merged into RTEMS's git `main` branch
via
...
`rtems-source-builder` bset, and several of its steps (the RSB bset name, the
OpenOCD build flags) are best-effort guesses that haven't actually been built in
this environment - cross-check each step against the
[riscv BSPs page](https://docs.rtems.org/docs/main/user/bsps/bsps-riscv.html)
before relying on it, and expect to iterate on the Dockerfile.
