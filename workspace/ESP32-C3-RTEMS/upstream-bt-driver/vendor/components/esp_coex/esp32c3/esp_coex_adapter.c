...
/* RTEMS compat tweak (2026-08-26): this port's BaseType_t is `long`
 * (freertos-compat/include/freertos/portmacro.h), not `int` like real
 * IDF's RISC-V FreeRTOS port - both are 32-bit on this target, but the
 * struct field below is typed `int (*)(void)`, so passing
 * xPortInIsrContext directly is a real signature mismatch, not just a
 * style choice. bt.c's own osi_funcs struct hits the same thing and
 * already uses a same-shaped local wrapper (`is_in_isr_wrapper`) -
 * matching that existing, working pattern here instead of changing
 * the shared BaseType_t typedef everywhere else. */
