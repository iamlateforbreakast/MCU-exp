...
| RP2040 | `workspace/RPI2040/examples/freertos_blink` | Runs FreeRTOS (single core, dynamic allocation): two tasks blink two LEDs at different rates, plus a producer/consumer task pair passing tick counts through a queue. Config and task/queue setup grounded against the official `pico-examples/freertos/hello_freertos` example. Requires `FREERTOS_KERNEL_PATH`, cloned automatically in the `rp2040-dev` container. |
